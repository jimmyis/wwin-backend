...
contract BATHMaster is Ownable, Withdrawable, ReentrancyGuard {
    ...
    uint256 public redeemFee = 5;
    uint256 public feeDenominator = 1000;
    
    event Mint(address indexed user, uint256 amount);
    event Redeem(address indexed user, uint256 amount);
    event BUSDWithdrawn(uint256 amount);
    event WWINWithdrawn(uint256 amount);
    event MintFeeChanged(uint256 fee);
    event RedeemFeeChanged(uint256 fee);
    event StrategistAddressChanged(address strategist);
    
    constructor(IBATH _bath, IERC20 _busd, IERC20 _wwin, address _routerAddress) public {
        require(
    ...
    function setMintFee(uint256 _fee) external onlyOwner {
        require(_fee >= 0, 'Fee must greater equal zero!');
        mintFee = _fee;
    ...
    function setRedeemFee(uint256 _fee) external onlyOwner {
        ...
        emit RedeemFeeChanged(redeemFee);
    }
    
    function mint(uint256 amount) external nonReentrant {
        require(amount <= busd.balanceOf(msg.sender), "BUSD is not enough");
        address[] memory pairToken = new address[](2);
        pairToken[0] = address(busd);
        pairToken[1] = address(bath);
        uint256[] memory amounts = IPancakeRouter(routerAddress).getAmountsOut(amount, pairToken);
        uint256 bathAmount = amounts[1];
        
        busd.safeTransferFrom(msg.sender, address(this), amount);
        uint256 feeAmount = amount * mintFee / feeDenominator;
        pairToken[0] = address(wwin);
        pairToken[1] = address(busd);
        uint256[] memory wwinAmounts = IPancakeRouter(routerAddress).getAmountsOut(feeAmount, pairToken);
        uint256 wwinAmount = wwinAmounts[0];
        require(wwinAmount <= wwin.balanceOf(msg.sender), "wWIN is not enough");

        wwin.safeTransferFrom(msg.sender, address(this), wwinAmount);
        bath.mint(msg.sender, bathAmount);
        
        emit Mint(msg.sender, bathAmount);
    }
    
    function redeem(uint256 amount) external nonReentrant {
        require(amount <= bath.balanceOf(msg.sender), "BATH is not enough");
        address[] memory pairToken = new address[](2);
        pairToken[0] = address(bath);
        pairToken[1] = address(busd);
        uint256[] memory amounts = IPancakeRouter(routerAddress).getAmountsOut(amount, pairToken);
        uint256 busdAmount = amounts[1];
        uint256 feeAmount = busdAmount * redeemFee / feeDenominator;
        pairToken[0] = address(busd);
        pairToken[1] = address(wwin);
        uint256[] memory wwinAmounts = IPancakeRouter(routerAddress).getAmountsOut(feeAmount, pairToken);
        uint256 wwinAmount = wwinAmounts[1];
        require(wwinAmount <= wwin.balanceOf(msg.sender), "wWIN is not enough");
    ...
    function withdrawWWIN(uint256 amount) external onlyOwner {
        wwin.safeTransfer(msg.sender, amount);
        
        emit WWINWithdrawn(amount);
    }
